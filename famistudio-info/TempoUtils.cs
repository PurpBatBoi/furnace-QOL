...
namespace FamiStudio
{
    static class FamiStudioTempoUtils
    {
        ...
        static Dictionary<CacheGrooveLengthKey, int[][]> cachedGroovePermutations = new Dictionary<CacheGrooveLengthKey, int[][]>();
        ...
        {
            Debug.Assert(ValidateGroove(groove));
            Debug.Assert(Platform.IsInMainThread());

            var key = new CacheGrooveLengthKey();
            key.groove = groove;
            if (cachedGroovePermutations.TryGetValue(key, out var permutations))
                return permutations;

            // Get all permutations
            var permutationsList = new List<int[]>();
            Utils.Permutations(groove, permutationsList);
            permutations = permutationsList.ToArray();

            cachedGroovePermutations.Add(key, permutations);
